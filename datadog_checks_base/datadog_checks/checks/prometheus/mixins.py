...
class PrometheusScraperMixin(object):
    ...
    def parse_metric_family(self, response):
        ...
        for metric in text_fd_to_metric_families(input_gen):
            ...
            #         messages[sample[self.SAMPLE_NAME]].append({"labels": sample[self.SAMPLE_LABELS], 'value': sample[self.SAMPLE_VALUE]})
            #     else:
            #         messages[metric_name].append({"labels": sample[self.SAMPLE_LABELS], 'value': sample[self.SAMPLE_VALUE]})
